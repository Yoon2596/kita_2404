...
RESULTS_PER_PAGE = 12 # (0718 MS 수정)

@app.route('/')
def index():
    locations = ALLOWED_LOCATIONS
    food_types= ALLOWED_FOOD_TYPES 
    shops = db.session.query(Web.ShopName, Web.ShopRating, Web.Reviews, Web.Address, Web.Address, Web.Category_catchtable, Web.LunchDinner, Web.Description).order_by(func.random()).limit(10).all()
...
def map_view(id):
    ...
    if shop:
        ...
        return render_template('map.html', shopName=shopName, address=address, shopRate=shopRate, reviews=reviews, nearby_shops=nearby_shops)
