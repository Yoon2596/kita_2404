...
def index():
    ...
    shops = db.session.query(Web.ShopName, Web.ShopRating, Web.Reviews, Web.Address, Web.Category_catchtable, Web.LunchDinner, Web.Description).order_by(func.random()).limit(10).all()
    return render_template('search.html', locations=locations, food_types=food_types, shops=shops)
...
def map_view(id):
    ...
    if shop:
        ...
        # Convert shop details and reviews for nearby shops to JSON serializable format
        nearby_shops_details = []
        nearby_shops_reviews = []
        for nearby_shop in nearby_shops:
            nearby_reviews = db.session.query(Web.Nickname, Web.User_Rating, Web.MealTime, Web.Date, Web.User_Review).filter_by(ShopName=nearby_shop.ShopName).all()
            nearby_reviews_list = [{
                "Nickname": review.Nickname,
                "User_Rating": review.User_Rating,
                "MealTime": review.MealTime,
                "Date": review.Date,
                "User_Review": review.User_Review
            } for review in nearby_reviews]
            nearby_shops_details.append({
                "ShopName": nearby_shop.ShopName,
                "ShopRating": nearby_shop.ShopRating,
                "Address": nearby_shop.Address,
                "LunchDinner": nearby_shop.LunchDinner,
                "Category": nearby_shop.Category_catchtable
            })
            nearby_shops_reviews.append({
                "index": len(nearby_shops_reviews) + 1,  # Ensure a 1-based index
                "reviews": nearby_reviews_list
            })

        return render_template('map.html', shopName=shopName, address=address, shopRate=shopRate, reviews=reviews,
                               price=price, description=description, category=category, user_reviews=user_reviews_list,
                               nearby_shops_details=nearby_shops_details, nearby_shops_reviews=nearby_shops_reviews)
